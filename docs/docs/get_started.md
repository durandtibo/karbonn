...
To make the package as slim as possible, only the packages required to use `karbonn` are installed.
...
```shell
pip install karbonn numpy
```

## Installing from source

To install `karbonn` from source, you can follow the steps below. First, you will need to
...
```shell
git clone git@github.com:durandtibo/karbonn.git
```
...
```shell
conda activate karbonn
```

This example uses `conda` to create a virtual environment, but you can use other tools or
configurations. Then, you should install the required package to use `karbonn` with the following
command:
