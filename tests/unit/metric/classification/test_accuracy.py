...
from karbonn.metric import BinaryAccuracy, EmptyMetricError
...
def test_binary_accuracy_state_default() -> None:
    assert isinstance(BinaryAccuracy()._state, AccuracyState)


def test_binary_accuracy_state_extended() -> None:
    assert isinstance(BinaryAccuracy(state=ExtendedAccuracyState())._state, ExtendedAccuracyState)
